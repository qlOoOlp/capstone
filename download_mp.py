...
import urllib.parse
import urllib.error
from tqdm import tqdm #! 다운 시각화 하려고 추가함
...
def get_release_scans(release_file):
    with urllib.request.urlopen(release_file) as scan_lines:
        scans = [scan_line.decode('utf-8').rstrip('\n') for scan_line in scan_lines]
    return scans
    ...
    for task_data_id in task_data:
        if task_data_id in TASK_FILES:
            for filepart in TASK_FILES[task_data_id]:
                ...
                download_file(url, localpath)
                print('Downloaded task data for ' + task_data_id)
...
def main():
    ...
    # download task data
    if args.task_data:
        task_data_to_download = set(args.task_data) & set(TASK_FILES.keys())
        if task_data_to_download:
            task_out_dir = os.path.join(args.out_dir, RELEASE_TASKS)
            download_task_data(task_data_to_download, task_out_dir)
        else:
            print('ERROR: Unrecognized task data id(s): ' + ', '.join(args.task_data))
        print('Done downloading task_data for ' + ', '.join(task_data_to_download))
    ...
    if args.id and args.id != 'ALL':  # download single scan
        ...
        if scan_id not in release_scans:
            ...
            scan_out_dir = os.path.join(args.out_dir, RELEASE, scan_id)
            download_scan(scan_id, scan_out_dir, file_types)
    else:  # download entire release
        warning_msg = 'WARNING: You are downloading the entire MP release which requires ' + RELEASE_SIZE + ' of space.'
        if len(file_types) != len(FILETYPES):
            warning_msg = 'WARNING: You are downloading all MP scans of type ' + ', '.join(file_types)
        print(warning_msg)
        print('Note that existing scan directories will be skipped. Delete partially downloaded directories to re-download.')
        print('***')
        input('Press any key to continue, or CTRL-C to exit.')
        release_out_dir = os.path.join(args.out_dir, RELEASE)
        download_release(release_scans, release_out_dir, file_types)
